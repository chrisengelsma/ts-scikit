...
export class FftPfa {
  ...
  static Transform(sign: number, nfft: number, z: number[]): void {
    ...
    for (let jfac = 0; jfac < this._NFAC; ++jfac) {
      ...
      let j13 = ( j12 + jinc ) % jmax;
      let j14 = ( j13 + jinc ) % jmax;
  ...
  private static _pfa5(z: number[], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let t2r = z[j2] + z[j3];
      let t2i = z[j2 + 1] + z[j3 + 1];
      let t3r = z[j1] - z[j4];
      let t3i = z[j1 + 1] - z[j4 + 1];
      let t4r = z[j2] - z[j3];
      let t4i = z[j2 + 1] - z[j3 + 1];
      let t5r = t1r + t2r;
      let t5i = t1i + t2i;
      let t6r = c1 * ( t1r - t2r );
      let t6i = c1 * ( t1i - t2i );
      let t7r = z[j0] - 0.25 * t5r;
      let t7i = z[j0 + 1] - 0.25 * t5i;
      let y1r = t7r + t6r;
      let y1i = t7i + t6i;
      let y2r = t7r - t6r;
      let y2i = t7i - t6i;
      let y3r = c3 * t3r - c2 * t4r;
  ...
  private static _pfa7(z: number[], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let t2r = z[j2] + z[j5];
      let t2i = z[j2 + 1] + z[j5 + 1];
      let t3r = z[j3] + z[j4];
      let t3i = z[j3 + 1] + z[j4 + 1];
      let t4r = z[j1] - z[j6];
      let t4i = z[j1 + 1] - z[j6 + 1];
      let t5r = z[j2] - z[j5];
      let t5i = z[j2 + 1] - z[j5 + 1];
      let t6r = z[j3] - z[j4];
      let t6i = z[j3 + 1] - z[j4 + 1];
      let t7r = z[j0] - 0.5 * t3r;
      let t7i = z[j0 + 1] - 0.5 * t3i;
      let t8r = t1r - t3r;
      let t8i = t1i - t3i;
      let t9r = t2r - t3r;
      let t9i = t2i - t3i;
      let y1r = t7r + c1 * t8r + c2 * t9r;
      let y1i = t7i + c1 * t8i + c2 * t9i;
      let y2r = t7r + c2 * t8r + c3 * t9r;
      let y2i = t7i + c2 * t8i + c3 * t9i;
      let y3r = t7r + c3 * t8r + c1 * t9r;
      let y3i = t7i + c3 * t8i + c1 * t9i;
      let y4r = c6 * t4r - c4 * t5r + c5 * t6r;
      let y4i = c6 * t4i - c4 * t5i + c5 * t6i;
      let y5r = c5 * t4r - c6 * t5r - c4 * t6r;
  ...
  private static _pfa8(z: number[], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let t2r = z[j0] - z[j4];
      let t2i = z[j0 + 1] - z[j4 + 1];
      let t3r = z[j1] + z[j5];
      let t3i = z[j1 + 1] + z[j5 + 1];
      let t4r = z[j1] - z[j5];
      let t4i = z[j1 + 1] - z[j5 + 1];
      let t5r = z[j2] + z[j6];
      let t5i = z[j2 + 1] + z[j6 + 1];
      let t6r = c1 * ( z[j2] - z[j6] );
      let t6i = c1 * ( z[j2 + 1] - z[j6 + 1] );
      let t7r = z[j3] + z[j7];
      let t7i = z[j3 + 1] + z[j7 + 1];
      let t8r = z[j3] - z[j7];
      let t8i = z[j3 + 1] - z[j7 + 1];
      let t9r = t1r + t5r;
      let t9i = t1i + t5i;
      let t10r = t3r + t7r;
      let t10i = t3i + t7i;
  ...
  private static _pfa9(z: number[], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let t2i = z[j0 + 1] - 0.5 * t1i;
      let t3r = c1 * ( z[j3] - z[j6] );
      let t3i = c1 * ( z[j3 + 1] - z[j6 + 1] );
      let t4r = z[j0] + t1r;
      let t4i = z[j0 + 1] + t1i;
      let t5r = z[j4] + z[j7];
      let t5i = z[j4 + 1] + z[j7 + 1];
      let t6r = z[j1] - 0.5 * t5r;
      let t6i = z[j1 + 1] - 0.5 * t5i;
      let t7r = z[j4] - z[j7];
      let t7i = z[j4 + 1] - z[j7 + 1];
      let t8r = z[j1] + t5r;
      let t8i = z[j1 + 1] + t5i;
      let t9r = z[j2] + z[j5];
      let t9i = z[j2 + 1] + z[j5 + 1];
      let t10r = z[j8] - 0.5 * t9r;
      let t10i = z[j8 + 1] - 0.5 * t9i;
      let t11r = z[j2] - z[j5];
      let t11i = z[j2 + 1] - z[j5 + 1];
      let t12r = z[j8] + t9r;
      let t12i = z[j8 + 1] + t9i;
      let t13r = t8r + t12r;
      let t13i = t8i + t12i;
      ...
      let t17r = t7r - t11r;
      let t17i = t7i - t11i;
      let t18r = c2 * t14r - c7 * t17r;
      let t18i = c2 * t14i - c7 * t17i;
      let t19r = c4 * t14r + c9 * t17r;
      let t19i = c4 * t14i + c9 * t17i;
      let t20r = c3 * t15r + c6 * t16r;
      let t20i = c3 * t15i + c6 * t16i;
      let t21r = c5 * t15r - c8 * t16r;
      let t21i = c5 * t15i - c8 * t16i;
      let t22r = t18r + t19r;
      let t22i = t18i + t19i;
      let t23r = t20r - t21r;
      let t23i = t20i - t21i;
      ...
      let y2r = t2r + t19r;
      let y2i = t2i + t19i;
      let y3r = t4r - 0.5 * t13r;
      let y3i = t4i - 0.5 * t13i;
      let y4r = t2r - t22r;
      let y4i = t2i - t22i;
      let y5r = t3r - t23r;
      let y5i = t3i - t23i;
  ...
  private static _pfa11(z: number[], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let t2r = z[j2] + z[j9];
      let t2i = z[j2 + 1] + z[j9 + 1];
      let t3r = z[j3] + z[j8];
      let t3i = z[j3 + 1] + z[j8 + 1];
      let t4r = z[j4] + z[j7];
      let t4i = z[j4 + 1] + z[j7 + 1];
      let t5r = z[j5] + z[j6];
      let t5i = z[j5 + 1] + z[j6 + 1];
      let t6r = z[j1] - z[j10];
      let t6i = z[j1 + 1] - z[j10 + 1];
      let t7r = z[j2] - z[j9];
      let t7i = z[j2 + 1] - z[j9 + 1];
      let t8r = z[j3] - z[j8];
      let t8i = z[j3 + 1] - z[j8 + 1];
      let t9r = z[j4] - z[j7];
      let t9i = z[j4 + 1] - z[j7 + 1];
      let t10r = z[j5] - z[j6];
      let t10i = z[j5 + 1] - z[j6 + 1];
      let t11r = z[j0] - 0.5 * t5r;
      let t11i = z[j0 + 1] - 0.5 * t5i;
      let t12r = t1r - t5r;
      let t12i = t1i - t5i;
      let t13r = t2r - t5r;
      let t13i = t2i - t5i;
      ...
      let t15r = t4r - t5r;
      let t15i = t4i - t5i;
      let y1r = t11r + c1 * t12r + c2 * t13r + c3 * t14r + c4 * t15r;
      let y1i = t11i + c1 * t12i + c2 * t13i + c3 * t14i + c4 * t15i;
      let y2r = t11r + c2 * t12r + c4 * t13r + c5 * t14r + c3 * t15r;
      let y2i = t11i + c2 * t12i + c4 * t13i + c5 * t14i + c3 * t15i;
      let y3r = t11r + c3 * t12r + c5 * t13r + c2 * t14r + c1 * t15r;
      let y3i = t11i + c3 * t12i + c5 * t13i + c2 * t14i + c1 * t15i;
      let y4r = t11r + c4 * t12r + c3 * t13r + c1 * t14r + c5 * t15r;
      let y4i = t11i + c4 * t12i + c3 * t13i + c1 * t14i + c5 * t15i;
      let y5r = t11r + c5 * t12r + c1 * t13r + c4 * t14r + c2 * t15r;
      let y5i = t11i + c5 * t12i + c1 * t13i + c4 * t14i + c2 * t15i;
      let y6r = c10 * t6r - c6 * t7r + c9 * t8r - c7 * t9r + c8 * t10r;
      let y6i = c10 * t6i - c6 * t7i + c9 * t8i - c7 * t9i + c8 * t10i;
      let y7r = c9 * t6r - c8 * t7r + c6 * t8r + c10 * t9r - c7 * t10r;
      let y7i = c9 * t6i - c8 * t7i + c6 * t8i + c10 * t9i - c7 * t10i;
      let y8r = c8 * t6r - c10 * t7r - c7 * t8r + c6 * t9r + c9 * t10r;
      let y8i = c8 * t6i - c10 * t7i - c7 * t8i + c6 * t9i + c9 * t10i;
      let y9r = c7 * t6r + c9 * t7r - c10 * t8r - c8 * t9r - c6 * t10r;
  ...
  private static _pfa13(z: number[], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let t2r = z[j2] + z[j11];
      let t2i = z[j2 + 1] + z[j11 + 1];
      let t3r = z[j3] + z[j10];
      let t3i = z[j3 + 1] + z[j10 + 1];
      let t4r = z[j4] + z[j9];
      let t4i = z[j4 + 1] + z[j9 + 1];
      let t5r = z[j5] + z[j8];
      let t5i = z[j5 + 1] + z[j8 + 1];
      let t6r = z[j6] + z[j7];
      let t6i = z[j6 + 1] + z[j7 + 1];
      let t7r = z[j1] - z[j12];
      let t7i = z[j1 + 1] - z[j12 + 1];
      let t8r = z[j2] - z[j11];
      let t8i = z[j2 + 1] - z[j11 + 1];
      let t9r = z[j3] - z[j10];
      let t9i = z[j3 + 1] - z[j10 + 1];
      let t10r = z[j4] - z[j9];
      let t10i = z[j4 + 1] - z[j9 + 1];
      let t11r = z[j5] - z[j8];
      let t11i = z[j5 + 1] - z[j8 + 1];
      let t12r = z[j6] - z[j7];
      let t12i = z[j6 + 1] - z[j7 + 1];
      let t13r = z[j0] - 0.5 * t6r;
      let t13i = z[j0 + 1] - 0.5 * t6i;
      let t14r = t1r - t6r;
      let t14i = t1i - t6i;
      let t15r = t2r - t6r;
      let t15i = t2i - t6i;
      ...
      let t18r = t5r - t6r;
      let t18i = t5i - t6i;
      let y1r = t13r + c1 * t14r + c2 * t15r + c3 * t16r + c4 * t17r + c5 * t18r;
      let y1i = t13i + c1 * t14i + c2 * t15i + c3 * t16i + c4 * t17i + c5 * t18i;
      let y2r = t13r + c2 * t14r + c4 * t15r + c6 * t16r + c5 * t17r + c3 * t18r;
      let y2i = t13i + c2 * t14i + c4 * t15i + c6 * t16i + c5 * t17i + c3 * t18i;
      let y3r = t13r + c3 * t14r + c6 * t15r + c4 * t16r + c1 * t17r + c2 * t18r;
      let y3i = t13i + c3 * t14i + c6 * t15i + c4 * t16i + c1 * t17i + c2 * t18i;
      let y4r = t13r + c4 * t14r + c5 * t15r + c1 * t16r + c3 * t17r + c6 * t18r;
      let y4i = t13i + c4 * t14i + c5 * t15i + c1 * t16i + c3 * t17i + c6 * t18i;
      let y5r = t13r + c5 * t14r + c3 * t15r + c2 * t16r + c6 * t17r + c1 * t18r;
      let y5i = t13i + c5 * t14i + c3 * t15i + c2 * t16i + c6 * t17i + c1 * t18i;
      let y6r = t13r + c6 * t14r + c1 * t15r + c5 * t16r + c2 * t17r + c4 * t18r;
      let y6i = t13i + c6 * t14i + c1 * t15i + c5 * t16i + c2 * t17i + c4 * t18i;
      let y7r = c12 * t7r - c7 * t8r + c11 * t9r - c8 * t10r + c10 * t11r - c9 * t12r;
      let y7i = c12 * t7i - c7 * t8i + c11 * t9i - c8 * t10i + c10 * t11i - c9 * t12i;
      let y8r = c11 * t7r - c9 * t8r + c8 * t9r - c12 * t10r - c7 * t11r + c10 * t12r;
      let y8i = c11 * t7i - c9 * t8i + c8 * t9i - c12 * t10i - c7 * t11i + c10 * t12i;
      let y9r = c10 * t7r - c11 * t8r - c7 * t9r + c9 * t10r - c12 * t11r - c8 * t12r;
      let y9i = c10 * t7i - c11 * t8i - c7 * t9i + c9 * t10i - c12 * t11i - c8 * t12i;
      let y10r = c9 * t7r + c12 * t8r - c10 * t9r - c7 * t10r + c8 * t11r + c11 * t12r;
      let y10i = c9 * t7i + c12 * t8i - c10 * t9i - c7 * t10i + c8 * t11i + c11 * t12i;
      let y11r = c8 * t7r + c10 * t8r + c12 * t9r - c11 * t10r - c9 * t11r - c7 * t12r;
  ...
  private static _pfa16(z: number[], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let t2r = z[j4] + z[j12];
      let t2i = z[j4 + 1] + z[j12 + 1];
      let t3r = z[j0] - z[j8];
      let t3i = z[j0 + 1] - z[j8 + 1];
      let t4r = c1 * ( z[j4] - z[j12] );
      let t4i = c1 * ( z[j4 + 1] - z[j12 + 1] );
      let t5r = t1r + t2r;
      let t5i = t1i + t2i;
      let t6r = t1r - t2r;
      let t6i = t1i - t2i;
      let t7r = z[j1] + z[j9];
      let t7i = z[j1 + 1] + z[j9 + 1];
      let t8r = z[j5] + z[j13];
      let t8i = z[j5 + 1] + z[j13 + 1];
      let t9r = z[j1] - z[j9];
      let t9i = z[j1 + 1] - z[j9 + 1];
      let t10r = z[j5] - z[j13];
      let t10i = z[j5 + 1] - z[j13 + 1];
      let t11r = t7r + t8r;
      let t11i = t7i + t8i;
      let t12r = t7r - t8r;
      let t12i = t7i - t8i;
      let t13r = z[j2] + z[j10];
      let t13i = z[j2 + 1] + z[j10 + 1];
      let t14r = z[j6] + z[j14];
      let t14i = z[j6 + 1] + z[j14 + 1];
      let t15r = z[j2] - z[j10];
      let t15i = z[j2 + 1] - z[j10 + 1];
      let t16r = z[j6] - z[j14];
      let t16i = z[j6 + 1] - z[j14 + 1];
      let t17r = t13r + t14r;
      let t17i = t13i + t14i;
      let t18r = c4 * ( t15r - t16r );
      let t18i = c4 * ( t15i - t16i );
      let t19r = c5 * ( t15r + t16r );
      let t19i = c5 * ( t15i + t16i );
      let t20r = c1 * ( t13r - t14r );
      let t20i = c1 * ( t13i - t14i );
      let t21r = z[j3] + z[j11];
      let t21i = z[j3 + 1] + z[j11 + 1];
      let t22r = z[j7] + z[j15];
      let t22i = z[j7 + 1] + z[j15 + 1];
      let t23r = z[j3] - z[j11];
      let t23i = z[j3 + 1] - z[j11 + 1];
      let t24r = z[j7] - z[j15];
      let t24i = z[j7 + 1] - z[j15 + 1];
      let t25r = t21r + t22r;
      let t25i = t21i + t22i;
      let t26r = t21r - t22r;
      let t26i = t21i - t22i;
      ...
      let t33r = t3r + t18r;
      let t33i = t3i + t18i;
      let t34r = c2 * t29r - c6 * t30r;
      let t34i = c2 * t29i - c6 * t30i;
      let t35r = t3r - t18r;
      let t35i = t3i - t18i;
      let t36r = c7 * t27r - c3 * t28r;
      let t36i = c7 * t27i - c3 * t28i;
      let t37r = t4r + t19r;
      let t37i = t4i + t19i;
      let t38r = c3 * t27r + c7 * t28r;
      let t38i = c3 * t27i + c7 * t28i;
      let t39r = t4r - t19r;
      let t39i = t4i - t19i;
      let t40r = c6 * t29r + c2 * t30r;
      let t40i = c6 * t29i + c2 * t30i;
      let t41r = c4 * ( t12r - t26r );
      let t41i = c4 * ( t12i - t26i );
      let t42r = c5 * ( t12r + t26r );
      let t42i = c5 * ( t12i + t26i );
      let y1r = t33r + t34r;
      let y1i = t33i + t34i;
      let y2r = t6r + t41r;
      let y2i = t6i + t41i;
      ...
      let y11r = t36r + t39r;
      let y11i = t36i + t39i;
      let y12r = c1 * ( t11r - t25r );
      let y12i = c1 * ( t11i - t25i );
      let y13r = t36r - t39r;
      let y13i = t36i - t39i;
      let y14r = t42r + t20r;
      let y14i = t42i + t20i;
  ...
  private static _pfa2a(n1: number, z: number[][], m: number, j0: number, j1: number) {
    ...
    for (let i = 0; i < m; ++i) {
      ...
      for (let i1 = 0; i1 < m1; i1 += 2) {
        let t1r = zj0[i1] - zj1[i1];
        let t1i = zj0[i1 + 1] - zj1[i1 + 1];
  ...
  private static _pfa3a(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let zj2: number[] = z[j2];
      for (let i1 = 0; i1 < m1; i1 += 2) {
        let t1r = zj1[i1] + zj2[i1];
        let t1i = zj1[i1 + 1] + zj2[i1 + 1];
        let y1r = zj0[i1] - 0.5 * t1r;
        let y1i = zj0[i1 + 1] - 0.5 * t1i;
        let y2r = c1 * ( zj1[i1] - zj2[i1] );
        let y2i = c1 * ( zj1[i1 + 1] - zj2[i1 + 1] );
        zj0[i1] = zj0[i1] + t1r;
  ...
  private static _pfa4a(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let zj3: number[] = z[j3];
      for (let i1 = 0; i1 < m1; i1 += 2) {
        let t1r = zj0[i1] + zj2[i1];
        let t1i = zj0[i1 + 1] + zj2[i1 + 1];
        let t2r = zj1[i1] + zj3[i1];
        let t2i = zj1[i1 + 1] + zj3[i1 + 1];
        let y1r = zj0[i1] - zj2[i1];
        let y1i = zj0[i1 + 1] - zj2[i1 + 1];
        let y3r = c1 * ( zj1[i1] - zj3[i1] );
        let y3i = c1 * ( zj1[i1 + 1] - zj3[i1 + 1] );
        zj0[i1] = t1r + t2r;
        zj0[i1 + 1] = t1i + t2i;
        zj1[i1] = y1r - y3i;
        zj1[i1 + 1] = y1i + y3r;
  ...
  private static _pfa5a(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let zj4: number[] = z[j4];
      for (let i1 = 0; i1 < m1; i1 += 2) {
        let t1r = zj1[i1] + zj4[i1];
        let t1i = zj1[i1 + 1] + zj4[i1 + 1];
        let t2r = zj2[i1] + zj3[i1];
        let t2i = zj2[i1 + 1] + zj3[i1 + 1];
        let t3r = zj1[i1] - zj4[i1];
        let t3i = zj1[i1 + 1] - zj4[i1 + 1];
        let t4r = zj2[i1] - zj3[i1];
        let t4i = zj2[i1 + 1] - zj3[i1 + 1];
        let t5r = t1r + t2r;
        let t5i = t1i + t2i;
        let t6r = c1 * ( t1r - t2r );
        let t6i = c1 * ( t1i - t2i );
        let t7r = zj0[i1] - 0.25 * t5r;
        let t7i = zj0[i1 + 1] - 0.25 * t5i;
        let y1r = t7r + t6r;
        let y1i = t7i + t6i;
        let y2r = t7r - t6r;
        let y2i = t7i - t6i;
  ...
  private static _pfa7a(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let zj6: number[] = z[j6];
      for (let i1 = 0; i1 < m1; i1 += 2) {
        let t1r = zj1[i1] + zj6[i1];
        let t1i = zj1[i1 + 1] + zj6[i1 + 1];
        let t2r = zj2[i1] + zj5[i1];
        let t2i = zj2[i1 + 1] + zj5[i1 + 1];
        let t3r = zj3[i1] + zj4[i1];
        let t3i = zj3[i1 + 1] + zj4[i1 + 1];
        let t4r = zj1[i1] - zj6[i1];
        let t4i = zj1[i1 + 1] - zj6[i1 + 1];
        let t5r = zj2[i1] - zj5[i1];
        let t5i = zj2[i1 + 1] - zj5[i1 + 1];
        let t6r = zj3[i1] - zj4[i1];
        let t6i = zj3[i1 + 1] - zj4[i1 + 1];
        let t7r = zj0[i1] - 0.5 * t3r;
        let t7i = zj0[i1 + 1] - 0.5 * t3i;
        let t8r = t1r - t3r;
        let t8i = t1i - t3i;
        let t9r = t2r - t3r;
        let t9i = t2i - t3i;
  ...
  private static _pfa8a(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let zj7: number[] = z[j7];
      for (let i1 = 0; i1 < m1; i1 += 2) {
        let t1r = zj0[i1] + zj4[i1];
        let t1i = zj0[i1 + 1] + zj4[i1 + 1];
        let t2r = zj0[i1] - zj4[i1];
        let t2i = zj0[i1 + 1] - zj4[i1 + 1];
        let t3r = zj1[i1] + zj5[i1];
        let t3i = zj1[i1 + 1] + zj5[i1 + 1];
        let t4r = zj1[i1] - zj5[i1];
        let t4i = zj1[i1 + 1] - zj5[i1 + 1];
        let t5r = zj2[i1] + zj6[i1];
        let t5i = zj2[i1 + 1] + zj6[i1 + 1];
        let t6r = c1 * ( zj2[i1] - zj6[i1] );
        let t6i = c1 * ( zj2[i1 + 1] - zj6[i1 + 1] );
        let t7r = zj3[i1] + zj7[i1];
        let t7i = zj3[i1 + 1] + zj7[i1 + 1];
        let t8r = zj3[i1] - zj7[i1];
        let t8i = zj3[i1 + 1] - zj7[i1 + 1];
        let t9r = t1r + t5r;
        let t9i = t1i + t5i;
        let t10r = t3r + t7r;
        let t10i = t3i + t7i;
  ...
  private static _pfa9a(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let zj8: number[] = z[j8];
      for (let i1 = 0; i1 < m1; i1 += 2) {
        let t1r = zj3[i1] + zj6[i1];
        let t1i = zj3[i1 + 1] + zj6[i1 + 1];
        let t2r = zj0[i1] - 0.5 * t1r;
        let t2i = zj0[i1 + 1] - 0.5 * t1i;
        let t3r = c1 * ( zj3[i1] - zj6[i1] );
        let t3i = c1 * ( zj3[i1 + 1] - zj6[i1 + 1] );
        let t4r = zj0[i1] + t1r;
        let t4i = zj0[i1 + 1] + t1i;
        let t5r = zj4[i1] + zj7[i1];
        let t5i = zj4[i1 + 1] + zj7[i1 + 1];
        let t6r = zj1[i1] - 0.5 * t5r;
        let t6i = zj1[i1 + 1] - 0.5 * t5i;
        let t7r = zj4[i1] - zj7[i1];
        let t7i = zj4[i1 + 1] - zj7[i1 + 1];
        let t8r = zj1[i1] + t5r;
        let t8i = zj1[i1 + 1] + t5i;
        let t9r = zj2[i1] + zj5[i1];
        let t9i = zj2[i1 + 1] + zj5[i1 + 1];
        let t10r = zj8[i1] - 0.5 * t9r;
        let t10i = zj8[i1 + 1] - 0.5 * t9i;
        let t11r = zj2[i1] - zj5[i1];
        let t11i = zj2[i1 + 1] - zj5[i1 + 1];
        let t12r = zj8[i1] + t9r;
        let t12i = zj8[i1 + 1] + t9i;
        let t13r = t8r + t12r;
        let t13i = t8i + t12i;
        let t14r = t6r + t10r;
        let t14i = t6i + t10i;
  ...
  private static _pfa11a(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let zj8: number[] = z[j8];
      let zj9: number[] = z[j9];
      let zj10: number[] = z[j10];
      for (let i1 = 0; i1 < m1; i1 += 2) {
        let t1r = zj1[i1] + zj10[i1];
        let t1i = zj1[i1 + 1] + zj10[i1 + 1];
        let t2r = zj2[i1] + zj9[i1];
        let t2i = zj2[i1 + 1] + zj9[i1 + 1];
        let t3r = zj3[i1] + zj8[i1];
        let t3i = zj3[i1 + 1] + zj8[i1 + 1];
        let t4r = zj4[i1] + zj7[i1];
        let t4i = zj4[i1 + 1] + zj7[i1 + 1];
        let t5r = zj5[i1] + zj6[i1];
        let t5i = zj5[i1 + 1] + zj6[i1 + 1];
        let t6r = zj1[i1] - zj10[i1];
        let t6i = zj1[i1 + 1] - zj10[i1 + 1];
        let t7r = zj2[i1] - zj9[i1];
        let t7i = zj2[i1 + 1] - zj9[i1 + 1];
        let t8r = zj3[i1] - zj8[i1];
        let t8i = zj3[i1 + 1] - zj8[i1 + 1];
        let t9r = zj4[i1] - zj7[i1];
        let t9i = zj4[i1 + 1] - zj7[i1 + 1];
        let t10r = zj5[i1] - zj6[i1];
        let t10i = zj5[i1 + 1] - zj6[i1 + 1];
        let t11r = zj0[i1] - 0.5 * t5r;
        let t11i = zj0[i1 + 1] - 0.5 * t5i;
        let t12r = t1r - t5r;
        let t12i = t1i - t5i;
        let t13r = t2r - t5r;
        let t13i = t2i - t5i;
  ...
  private static _pfa13a(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let zj9: number[] = z[j9];
      let zj10: number[] = z[j10];
      let zj11: number[] = z[j11];
      let zj12: number[] = z[j12];
      for (let i1 = 0; i1 < m1; i1 += 2) {
        let t1r = zj1[i1] + zj12[i1];
        let t1i = zj1[i1 + 1] + zj12[i1 + 1];
        let t2r = zj2[i1] + zj11[i1];
        let t2i = zj2[i1 + 1] + zj11[i1 + 1];
        let t3r = zj3[i1] + zj10[i1];
        let t3i = zj3[i1 + 1] + zj10[i1 + 1];
        let t4r = zj4[i1] + zj9[i1];
        let t4i = zj4[i1 + 1] + zj9[i1 + 1];
        let t5r = zj5[i1] + zj8[i1];
        let t5i = zj5[i1 + 1] + zj8[i1 + 1];
        let t6r = zj6[i1] + zj7[i1];
        let t6i = zj6[i1 + 1] + zj7[i1 + 1];
        let t7r = zj1[i1] - zj12[i1];
        let t7i = zj1[i1 + 1] - zj12[i1 + 1];
        let t8r = zj2[i1] - zj11[i1];
        let t8i = zj2[i1 + 1] - zj11[i1 + 1];
        let t9r = zj3[i1] - zj10[i1];
        let t9i = zj3[i1 + 1] - zj10[i1 + 1];
        let t10r = zj4[i1] - zj9[i1];
        let t10i = zj4[i1 + 1] - zj9[i1 + 1];
        let t11r = zj5[i1] - zj8[i1];
        let t11i = zj5[i1 + 1] - zj8[i1 + 1];
        let t12r = zj6[i1] - zj7[i1];
        let t12i = zj6[i1 + 1] - zj7[i1 + 1];
        let t13r = zj0[i1] - 0.5 * t6r;
        let t13i = zj0[i1 + 1] - 0.5 * t6i;
        let t14r = t1r - t6r;
        let t14i = t1i - t6i;
        let t15r = t2r - t6r;
        let t15i = t2i - t6i;
  ...
  private static _pfa16a(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let zj11: number[] = z[j11];
      let zj12: number[] = z[j12];
      let zj13: number[] = z[j13];
      let zj14: number[] = z[j14];
      let zj15: number[] = z[j15];
      for (let i1 = 0; i1 < m1; i1 += 2) {
        let t1r = zj0[i1] + zj8[i1];
        let t1i = zj0[i1 + 1] + zj8[i1 + 1];
        let t2r = zj4[i1] + zj12[i1];
        let t2i = zj4[i1 + 1] + zj12[i1 + 1];
        let t3r = zj0[i1] - zj8[i1];
        let t3i = zj0[i1 + 1] - zj8[i1 + 1];
        let t4r = c1 * ( zj4[i1] - zj12[i1] );
        let t4i = c1 * ( zj4[i1 + 1] - zj12[i1 + 1] );
        let t5r = t1r + t2r;
        let t5i = t1i + t2i;
        let t6r = t1r - t2r;
        let t6i = t1i - t2i;
        let t7r = zj1[i1] + zj9[i1];
        let t7i = zj1[i1 + 1] + zj9[i1 + 1];
        let t8r = zj5[i1] + zj13[i1];
        let t8i = zj5[i1 + 1] + zj13[i1 + 1];
        let t9r = zj1[i1] - zj9[i1];
        let t9i = zj1[i1 + 1] - zj9[i1 + 1];
        let t10r = zj5[i1] - zj13[i1];
        let t10i = zj5[i1 + 1] - zj13[i1 + 1];
        let t11r = t7r + t8r;
        let t11i = t7i + t8i;
        let t12r = t7r - t8r;
        let t12i = t7i - t8i;
        let t13r = zj2[i1] + zj10[i1];
        let t13i = zj2[i1 + 1] + zj10[i1 + 1];
        let t14r = zj6[i1] + zj14[i1];
        let t14i = zj6[i1 + 1] + zj14[i1 + 1];
        let t15r = zj2[i1] - zj10[i1];
        let t15i = zj2[i1 + 1] - zj10[i1 + 1];
        let t16r = zj6[i1] - zj14[i1];
        let t16i = zj6[i1 + 1] - zj14[i1 + 1];
        let t17r = t13r + t14r;
        let t17i = t13i + t14i;
        let t18r = c4 * ( t15r - t16r );
        let t18i = c4 * ( t15i - t16i );
        ...
        let t20r = c1 * ( t13r - t14r );
        let t20i = c1 * ( t13i - t14i );
        let t21r = zj3[i1] + zj11[i1];
        let t21i = zj3[i1 + 1] + zj11[i1 + 1];
        let t22r = zj7[i1] + zj15[i1];
        let t22i = zj7[i1 + 1] + zj15[i1 + 1];
        let t23r = zj3[i1] - zj11[i1];
        let t23i = zj3[i1 + 1] - zj11[i1 + 1];
        let t24r = zj7[i1] - zj15[i1];
        let t24i = zj7[i1 + 1] - zj15[i1 + 1];
        let t25r = t21r + t22r;
        let t25i = t21i + t22i;
        let t26r = t21r - t22r;
        let t26i = t21i - t22i;
  ...
  private static _pfa3b(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      for (let i1 = 0; i1 < n1; ++i1) {
        ...
        let y1i = zj0i[i1] - 0.5 * t1i;
        let y2r = c1 * ( zj1r[i1] - zj2r[i1] );
        let y2i = c1 * ( zj1i[i1] - zj2i[i1] );
        zj0r[i1] = zj0r[i1] + t1r;
        zj0i[i1] = zj0i[i1] + t1i;
        zj1r[i1] = y1r - y2i;
        zj1i[i1] = y1i + y2r;
  ...
  private static _pfa4b(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      for (let i1 = 0; i1 < n1; ++i1) {
        ...
        let t2i = zj1i[i1] + zj3i[i1];
        let y1r = zj0r[i1] - zj2r[i1];
        let y1i = zj0i[i1] - zj2i[i1];
        let y3r = c1 * ( zj1r[i1] - zj3r[i1] );
        let y3i = c1 * ( zj1i[i1] - zj3i[i1] );
        zj0r[i1] = t1r + t2r;
        zj0i[i1] = t1i + t2i;
        zj1r[i1] = y1r - y3i;
        zj1i[i1] = y1i + y3r;
  ...
  private static _pfa5b(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      for (let i1 = 0; i1 < n1; ++i1) {
        ...
        let t2i = zj2i[i1] + zj3i[i1];
        let t3r = zj1r[i1] - zj4r[i1];
        let t3i = zj1i[i1] - zj4i[i1];
        let t4r = zj2r[i1] - zj3r[i1];
        let t4i = zj2i[i1] - zj3i[i1];
        let t5r = t1r + t2r;
        let t5i = t1i + t2i;
        let t6r = c1 * ( t1r - t2r );
        let t6i = c1 * ( t1i - t2i );
        let t7r = zj0r[i1] - 0.25 * t5r;
        let t7i = zj0i[i1] - 0.25 * t5i;
        let y1r = t7r + t6r;
        let y1i = t7i + t6i;
        let y2r = t7r - t6r;
        let y2i = t7i - t6i;
  ...
  private static _pfa7b(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      for (let i1 = 0; i1 < n1; ++i1) {
        ...
        let t2i = zj2i[i1] + zj5i[i1];
        let t3r = zj3r[i1] + zj4r[i1];
        let t3i = zj3i[i1] + zj4i[i1];
        let t4r = zj1r[i1] - zj6r[i1];
        let t4i = zj1i[i1] - zj6i[i1];
        let t5r = zj2r[i1] - zj5r[i1];
        let t5i = zj2i[i1] - zj5i[i1];
        let t6r = zj3r[i1] - zj4r[i1];
        let t6i = zj3i[i1] - zj4i[i1];
        let t7r = zj0r[i1] - 0.5 * t3r;
        let t7i = zj0i[i1] - 0.5 * t3i;
        let t8r = t1r - t3r;
        let t8i = t1i - t3i;
        let t9r = t2r - t3r;
        let t9i = t2i - t3i;
  ...
  private static _pfa8b(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      for (let i1 = 0; i1 < n1; ++i1) {
        ...
        let t2i = zj0i[i1] - zj4i[i1];
        let t3r = zj1r[i1] + zj5r[i1];
        let t3i = zj1i[i1] + zj5i[i1];
        let t4r = zj1r[i1] - zj5r[i1];
        let t4i = zj1i[i1] - zj5i[i1];
        let t5r = zj2r[i1] + zj6r[i1];
        let t5i = zj2i[i1] + zj6i[i1];
        let t6r = c1 * ( zj2r[i1] - zj6r[i1] );
        let t6i = c1 * ( zj2i[i1] - zj6i[i1] );
        let t7r = zj3r[i1] + zj7r[i1];
        let t7i = zj3i[i1] + zj7i[i1];
        let t8r = zj3r[i1] - zj7r[i1];
        let t8i = zj3i[i1] - zj7i[i1];
        let t9r = t1r + t5r;
        let t9i = t1i + t5i;
        let t10r = t3r + t7r;
        let t10i = t3i + t7i;
  ...
  private static _pfa9b(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      for (let i1 = 0; i1 < n1; ++i1) {
        ...
        let t2i = zj0i[i1] - 0.5 * t1i;
        let t3r = c1 * ( zj3r[i1] - zj6r[i1] );
        let t3i = c1 * ( zj3i[i1] - zj6i[i1] );
        let t4r = zj0r[i1] + t1r;
        let t4i = zj0i[i1] + t1i;
        let t5r = zj4r[i1] + zj7r[i1];
        let t5i = zj4i[i1] + zj7i[i1];
        let t6r = zj1r[i1] - 0.5 * t5r;
        let t6i = zj1i[i1] - 0.5 * t5i;
        let t7r = zj4r[i1] - zj7r[i1];
        let t7i = zj4i[i1] - zj7i[i1];
        let t8r = zj1r[i1] + t5r;
        let t8i = zj1i[i1] + t5i;
        let t9r = zj2r[i1] + zj5r[i1];
        let t9i = zj2i[i1] + zj5i[i1];
        let t10r = zj8r[i1] - 0.5 * t9r;
        let t10i = zj8i[i1] - 0.5 * t9i;
        let t11r = zj2r[i1] - zj5r[i1];
        let t11i = zj2i[i1] - zj5i[i1];
        let t12r = zj8r[i1] + t9r;
        let t12i = zj8i[i1] + t9i;
        let t13r = t8r + t12r;
        let t13i = t8i + t12i;
        let t14r = t6r + t10r;
        let t14i = t6i + t10i;
  ...
  private static _pfa11b(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      for (let i1 = 0; i1 < n1; ++i1) {
        ...
        let t2i = zj2i[i1] + zj9i[i1];
        let t3r = zj3r[i1] + zj8r[i1];
        let t3i = zj3i[i1] + zj8i[i1];
        let t4r = zj4r[i1] + zj7r[i1];
        let t4i = zj4i[i1] + zj7i[i1];
        let t5r = zj5r[i1] + zj6r[i1];
        let t5i = zj5i[i1] + zj6i[i1];
        let t6r = zj1r[i1] - zj10r[i1];
        let t6i = zj1i[i1] - zj10i[i1];
        let t7r = zj2r[i1] - zj9r[i1];
        let t7i = zj2i[i1] - zj9i[i1];
        let t8r = zj3r[i1] - zj8r[i1];
        let t8i = zj3i[i1] - zj8i[i1];
        let t9r = zj4r[i1] - zj7r[i1];
        let t9i = zj4i[i1] - zj7i[i1];
        let t10r = zj5r[i1] - zj6r[i1];
        let t10i = zj5i[i1] - zj6i[i1];
        let t11r = zj0r[i1] - 0.5 * t5r;
        let t11i = zj0i[i1] - 0.5 * t5i;
        let t12r = t1r - t5r;
        let t12i = t1i - t5i;
        let t13r = t2r - t5r;
        let t13i = t2i - t5i;
  ...
  private static _pfa13b(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      for (let i1 = 0; i1 < n1; ++i1) {
        ...
        let t2i = zj2i[i1] + zj11i[i1];
        let t3r = zj3r[i1] + zj10r[i1];
        let t3i = zj3i[i1] + zj10i[i1];
        let t4r = zj4r[i1] + zj9r[i1];
        let t4i = zj4i[i1] + zj9i[i1];
        let t5r = zj5r[i1] + zj8r[i1];
        let t5i = zj5i[i1] + zj8i[i1];
        let t6r = zj6r[i1] + zj7r[i1];
        let t6i = zj6i[i1] + zj7i[i1];
        let t7r = zj1r[i1] - zj12r[i1];
        let t7i = zj1i[i1] - zj12i[i1];
        let t8r = zj2r[i1] - zj11r[i1];
        let t8i = zj2i[i1] - zj11i[i1];
        let t9r = zj3r[i1] - zj10r[i1];
        let t9i = zj3i[i1] - zj10i[i1];
        let t10r = zj4r[i1] - zj9r[i1];
        let t10i = zj4i[i1] - zj9i[i1];
        let t11r = zj5r[i1] - zj8r[i1];
        let t11i = zj5i[i1] - zj8i[i1];
        let t12r = zj6r[i1] - zj7r[i1];
        let t12i = zj6i[i1] - zj7i[i1];
        let t13r = zj0r[i1] - 0.5 * t6r;
        let t13i = zj0i[i1] - 0.5 * t6i;
        let t14r = t1r - t6r;
        let t14i = t1i - t6i;
        let t15r = t2r - t6r;
        let t15i = t2i - t6i;
  ...
  private static _pfa16b(n1: number, z: number[][], mu: number, m: number,
    ...
    for (let i = 0; i < m; ++i) {
      ...
      let zj1r: number[] = z[j1];
      let zj1i: number[] = z[j1 + 1];
      let zj2r: number[] = z[j2];
      let zj2i: number[] = z[j2 + 1];
      let zj3r: number[] = z[j3];
      let zj3i: number[] = z[j3 + 1];
      let zj4r: number[] = z[j4];
      let zj4i: number[] = z[j4 + 1];
      let zj5r: number[] = z[j5];
      let zj5i: number[] = z[j5 + 1];
      let zj6r: number[] = z[j6];
      let zj6i: number[] = z[j6 + 1];
      let zj7r: number[] = z[j7];
      let zj7i: number[] = z[j7 + 1];
      let zj8r: number[] = z[j8];
      let zj8i: number[] = z[j8 + 1];
      let zj9r: number[] = z[j9];
      let zj9i: number[] = z[j9 + 1];
      let zj10r: number[] = z[j10];
      let zj10i: number[] = z[j10 + 1];
      let zj11r: number[] = z[j11];
      let zj11i: number[] = z[j11 + 1];
      let zj12r: number[] = z[j12];
      let zj12i: number[] = z[j12 + 1];
      let zj13r: number[] = z[j13];
      let zj13i: number[] = z[j13 + 1];
      let zj14r: number[] = z[j14];
      ...
      for (let i1 = 0; i1 < n1; ++i1) {
        ...
        let t2i = zj4i[i1] + zj12i[i1];
        let t3r = zj0r[i1] - zj8r[i1];
        let t3i = zj0i[i1] - zj8i[i1];
        let t4r = c1 * ( zj4r[i1] - zj12r[i1] );
        let t4i = c1 * ( zj4i[i1] - zj12i[i1] );
        let t5r = t1r + t2r;
        let t5i = t1i + t2i;
        let t6r = t1r - t2r;
        let t6i = t1i - t2i;
        let t7r = zj1r[i1] + zj9r[i1];
        let t7i = zj1i[i1] + zj9i[i1];
        let t8r = zj5r[i1] + zj13r[i1];
        let t8i = zj5i[i1] + zj13i[i1];
        let t9r = zj1r[i1] - zj9r[i1];
        let t9i = zj1i[i1] - zj9i[i1];
        let t10r = zj5r[i1] - zj13r[i1];
        let t10i = zj5i[i1] - zj13i[i1];
        let t11r = t7r + t8r;
        let t11i = t7i + t8i;
        let t12r = t7r - t8r;
        let t12i = t7i - t8i;
        let t13r = zj2r[i1] + zj10r[i1];
        let t13i = zj2i[i1] + zj10i[i1];
        let t14r = zj6r[i1] + zj14r[i1];
        let t14i = zj6i[i1] + zj14i[i1];
        let t15r = zj2r[i1] - zj10r[i1];
        let t15i = zj2i[i1] - zj10i[i1];
        let t16r = zj6r[i1] - zj14r[i1];
        let t16i = zj6i[i1] - zj14i[i1];
        let t17r = t13r + t14r;
        let t17i = t13i + t14i;
        let t18r = c4 * ( t15r - t16r );
        let t18i = c4 * ( t15i - t16i );
        ...
        let t20r = c1 * ( t13r - t14r );
        let t20i = c1 * ( t13i - t14i );
        let t21r = zj3r[i1] + zj11r[i1];
        let t21i = zj3i[i1] + zj11i[i1];
        let t22r = zj7r[i1] + zj15r[i1];
        let t22i = zj7i[i1] + zj15i[i1];
        let t23r = zj3r[i1] - zj11r[i1];
        let t23i = zj3i[i1] - zj11i[i1];
        let t24r = zj7r[i1] - zj15r[i1];
        let t24i = zj7i[i1] - zj15i[i1];
        let t25r = t21r + t22r;
        let t25i = t21i + t22i;
        let t26r = t21r - t22r;
        let t26i = t21i - t22i;
